...
`StagingFs` (`filesystem/staging.rs`) is a journaled, transactional `FileSystem` decorator: reads delegate to the inner FS (so staged writes are never visible through `read`); every write-side call streams into a temp file under the `temp_dir` passed to `StagingFs::new` (a policy of the decorator, not a port method: production supplies `cli::staging_dir()` = `std::env::temp_dir()`, tests a literal `/tmp`) or appends a `Rename`/`Remove` entry to an in-memory journal. The temp is deliberately **not** beside its target: `search` over the tree never sees half-written output, and an interrupted run leaves nothing in the user's directories. `commit` replays the journal in order (each `Write` becomes `inner.rename(temp, target)`); dropping without commit deletes the temps and discards the journal, leaving the inner FS untouched. `PhysicalFS::rename` tries `rename(2)` first and, on `ErrorKind::CrossesDevices` (temp dir on a tmpfs), copies to a sibling of the target and renames that, so the target still changes in one step. **Every file bulked writes goes through `StagingFs`**: apply and refresh's rewrites, and the `-o` output of search, ingest and refresh, all via `cli::write_file_atomically`. Nothing calls `File::create` on a user path directly.
